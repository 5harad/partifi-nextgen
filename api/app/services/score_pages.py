...
def ensure_score_pages_warming(score_id: str) -> dict[str, bool]:
    """Return images_ready / images_warming; enqueue warm_score_pages when needed."""
    if score_pages_available(score_id):
        return {"images_ready": True, "images_warming": False}

    if try_acquire_score_pages_lock(score_id):
        enqueue_job("warm_score_pages", {"score_id": score_id})

    return {"images_ready": False, "images_warming": True}
